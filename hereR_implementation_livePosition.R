...
set_key(Sys.getenv("here_api_key_v2"))
...
# merge the archived json files vs new parquet
...
inbound_sf <- sf::st_as_sf(inbound_df %>% ungroup(), coords = c("Longitude", "Latitude"), remove = FALSE)

st_crs(inbound_sf) <- "WGS84"

ibese_sf <- sf::st_as_sf(ibese_df, coords = c("lon", "lat"), remove = FALSE)

st_crs(ibese_sf) <- "WGS84"

pred_arrival <- route_matrix(
  origin = inbound_sf,
  destination = ibese_sf,
  url_only = FALSE,
  transport_mode = "truck"
) %>% 
  select(departure, arrival, distance, duration) %>% 
  bind_cols(inbound_df %>% select(-LongLat, -DistToPlant, -DistCovered, -TimeDiff, -Direction)) %>% 
  select(Reference, TripID, Longitude, Latitude, Geofence, Altitude, DateTimeReceived, Departure = departure, Distance = distance, Duration = duration, Arrival = arrival)
